...
class AuthenticationService {
    ...
    private getJWTToken = async (
        email: string
    ): Promise<{
        data: JWTCredentialDto[];
        total: number;
    }> => {
        return await get({
    ...
    private createJWT = async (email: string): Promise<JWTCredentialDto> => {
        return await post({
            url: `http://kong:8001/consumers/${email}/jwt`,
            formData: {},
            json: true
    ...
    // login
    public login = async (loginData: LoginDto): Promise<string> => {
        // check if user exists
